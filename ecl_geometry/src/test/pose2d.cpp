...
TEST(Pose2DTests,RotationMatrixConstructors) {
	Pose2D<double> pose1;
	...
	Pose2D<double> pose2(1.0, 2.0, 3.14);
	...
	Pose2D<double> pose4(3.14,trans);
	...
	Pose2D<double,RotationAngleStorage> pose_other(3.14,trans);
	Pose2D<double> pose6(pose_other);
...
TEST(Pose2DTests,RotationAngleConstructors) {
	Pose2D<double,RotationAngleStorage> pose1;
	EXPECT_EQ(0.0,pose1.x());
	EXPECT_EQ(0.0,pose1.y());
	EXPECT_GT(0.01,pose1.rotation());
	EXPECT_LT(-0.01,pose1.rotation());
	Pose2D<double,RotationAngleStorage> pose2(1.0, 2.0, 3.14);
	EXPECT_EQ(1.0,pose2.x());
	EXPECT_EQ(2.0,pose2.y());
	EXPECT_EQ(3.14,pose2.heading());
	Vector2d trans; trans << 1.0, 2.0;
	Matrix2d rot = Angle<double>(3.14).rotationMatrix();
	Pose2D<double,RotationAngleStorage> pose3(rot,trans);
	EXPECT_EQ(1.0,pose3.x());
	EXPECT_EQ(2.0,pose3.y());
	EXPECT_GT(3.15, pose3.heading());// Allow some roundoff error here
	EXPECT_LT(3.13, pose3.heading());
	Pose2D<double,RotationAngleStorage> pose4(3.14,trans);
	EXPECT_EQ(1.0,pose4.x());
	EXPECT_EQ(2.0,pose4.y());
	EXPECT_GT(3.15, pose4.heading());// Allow some roundoff error here
	EXPECT_LT(3.13, pose4.heading());
	Pose2D<double,RotationAngleStorage> pose5(pose4);
	EXPECT_EQ(1.0,pose5.x());
	EXPECT_EQ(2.0,pose5.y());
	EXPECT_GT(3.15, pose5.heading());// Allow some roundoff error here
	EXPECT_LT(3.13, pose5.heading());
	Pose2D<double> pose_other(1.0, 2.0, 3.14);
	Pose2D<double,RotationAngleStorage> pose6(pose_other);
	EXPECT_EQ(1.0,pose6.x());
	EXPECT_EQ(2.0,pose6.y());
	EXPECT_GT(3.15, pose6.heading());// Allow some roundoff error here
	EXPECT_LT(3.13, pose6.heading());
...
TEST(Pose2DTests,assignment) {
	Pose2D<double> pose_r1(2.0, 1.0, 1.14);
	Pose2D<double,RotationAngleStorage> pose_a1(1.0, 2.0, 3.14);
	Pose2D<double> pose_r2;
	Pose2D<double,RotationAngleStorage> pose_a2;
	pose_r2 = pose_r1;
...
TEST(Pose2DTests,eigenStyle) {
	...
	Pose2D<double,RotationAngleStorage> apose;
	apose.rotation(3.14);
...
TEST(Pose2DTests,convenienceStyle) {
	Vector2d trans; trans << 1.0, 2.0;
	Matrix2d rot;
	Pose2D<double> rpose;
...
TEST(Pose2DTests,inverse) {
	Pose2D<double> pose(1.0, 2.0, ecl::pi);
	Pose2D<double> inverse = pose.inverse();
	...
	Pose2D<double,RotationAngleStorage> apose(1.0, 2.0, ecl::pi);
	Pose2D<double,RotationAngleStorage> ainverse = pose.inverse();
...
TEST(Pose2DTests,operators) {
	Pose2D<double> a(1.0, 2.0, 0.0), b(1.0, 3.0, ecl::pi);
	Pose2D<double> diff = a.inverse()*b; //	diff = b - a;
	EXPECT_EQ(0.0,diff.x());
	EXPECT_EQ(1.0,diff.y());
	EXPECT_EQ(ecl::pi,diff.heading());
	Pose2D<double,RotationAngleStorage> a2(1.0, 2.0, 0.0), b2(1.0, 3.0, ecl::pi);
	Pose2D<double,RotationAngleStorage> diff2 = a2.inverse()*b2; //	diff = b - a;
	EXPECT_EQ(0.0,diff2.x());
	EXPECT_EQ(1.0,diff2.y());
	EXPECT_EQ(ecl::pi,diff2.heading());
}
TEST(Pose2DTests,relative) {
	...
	Pose2D<double,RotationAngleStorage> a_(1.0, 1.0, 1.57), b_(1.0, 2.0, 3.14);
	Pose2D<double,RotationAngleStorage> brela_ = b.relative(a);
